
-- Add subscription tier enum
CREATE TYPE public.subscription_tier AS ENUM ('free', 'basic', 'premium');

-- Add subscription columns to profiles
ALTER TABLE public.profiles 
  ADD COLUMN subscription_tier subscription_tier NOT NULL DEFAULT 'basic',
  ADD COLUMN payment_status text NOT NULL DEFAULT 'pending',
  ADD COLUMN subscription_note text;

-- Coaches are always free
UPDATE public.profiles SET subscription_tier = 'free', payment_status = 'active' WHERE role = 'coach';

-- Update handle_new_user to support subscription tier
CREATE OR REPLACE FUNCTION public.handle_new_user()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
DECLARE
  _role app_role;
  _approved boolean;
  _coach_count integer;
  _coach_id uuid;
  _tier subscription_tier;
  _payment text;
BEGIN
  _role := COALESCE((NEW.raw_user_meta_data->>'role')::app_role, 'player');
  _coach_id := (NEW.raw_user_meta_data->>'coach_id')::uuid;
  
  -- Check if this is the first coach (auto-approve)
  SELECT COUNT(*) INTO _coach_count FROM public.profiles WHERE role = 'coach';
  
  IF _role = 'coach' THEN
    _tier := 'free';
    _payment := 'active';
    IF _coach_count = 0 THEN
      _approved := true;
    ELSE
      _approved := false;
    END IF;
  ELSE
    -- Player tier from metadata
    _tier := COALESCE((NEW.raw_user_meta_data->>'subscription_tier')::subscription_tier, 'basic');
    _payment := 'pending';
    _approved := false;
  END IF;

  INSERT INTO public.profiles (user_id, display_name, role, is_approved, coach_id, subscription_tier, payment_status)
  VALUES (
    NEW.id,
    COALESCE(NEW.raw_user_meta_data->>'display_name', NEW.email),
    _role,
    _approved,
    _coach_id,
    _tier,
    _payment
  );
  
  INSERT INTO public.user_roles (user_id, role)
  VALUES (NEW.id, _role);
  
  RETURN NEW;
END;
$function$;
