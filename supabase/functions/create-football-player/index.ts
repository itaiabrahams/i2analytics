...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // football_enabled defaults to false so players from the basketball apps
    // cannot log in here. This is the one place that flips it on.
    if (newUser?.user) {
      await adminClient
        .from("profiles")
        .update({ team, position, age: age ? parseInt(age) : null, is_approved: true, football_enabled: true })
